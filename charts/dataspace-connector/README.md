...
| postgresql | object | `{"enabled":true,"external":false,"postgresqlDatabase":"test","postgresqlHost":null,"postgresqlPassword":"username","postgresqlUsername":"password","service":{"port":"5432"}}` | Persistent database properties |
| postgresql.enabled | bool | `true` | Whether to deploy a postgresql database server alongside the dataspace-connector |
| postgresql.external | bool | `false` | Whether to use an existing postgresql database rather than deploying a new one. Only set this to true, if "enabled" is false. |
| postgresql.postgresqlDatabase | string | `"test"` | Postgresql database name |
| postgresql.postgresqlHost | string | `nil` | The hostname of the external postgresql database. |
